...
// Função para imprimir cabeçalho estilizado
...
// Função que executa um comando e captura a saída em string
...
// Função que checa se o pacote existe no APT
bool pacoteExisteAPT(const std::string& pacote) {
    std::string comando = "apt search " + pacote;
    std::string output = executarComando(comando + " 2>/dev/null");
    // Se a saída contiver o nome do pacote, considera que existe
