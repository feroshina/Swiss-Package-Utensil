...
struct Manager
{
    ...
    std::string search;
    int trust; // 1 = seguro, 2 = semi, 3 = não recomendado
};
...
bool isUnsafeManager(const std::string& name)
{
    return getManagerTrust(name) >= 3;
}
...
bool confirmUnsafe(const std::string& managerName)
{
    ...
    std::cout << "Continuar mesmo assim? (y/n): ";
...
std::vector<std::string> getPriority()
{
    ...
    if (family == "arch")
        return {"pacman", "flatpak", "snap", "yay", "paru"};
